...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if rules := st.evm.ChainConfig().FlareRules(st.evm.Context.BlockNumber, st.evm.Context.Time); rules.IsApricotPhase2 {
	...
	if vmerr == nil && msg.To() != nil && *msg.To() == prioritised {
		...
		if st.evm.ChainConfig().IsFlareHardFork1(st.evm.Context.Time) {
			nominalGasPrice = uint64(params.ApricotPhase4MinBaseFee)
		}
